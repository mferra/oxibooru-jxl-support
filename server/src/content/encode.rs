...
pub fn to_jxl(image: &DynamicImage) -> ImageResult<Vec<u8>> {
    ...
    let encoder = encoder_builder()
        .build()
        .map_err(|e| ImageError::Encoding(EncodingError::new(ImageFormatHint::Exact(ImageFormat::Jxl), Box::new(e))))?;

    let frame = EncoderFrame::new(rgb.as_raw()).num_channels(3);
    encoder
        .encode_frame::<u8, u8>(&frame, width, height)
        .map(|output| Vec::from(output.as_ref()))
        .map_err(|e| {
            ImageError::Encoding(EncodingError::new(ImageFormatHint::Exact(ImageFormat::Jxl), Box::new(e)))
        })
}
